...
export function cargarEstructura1(container) {
    ...
    function createLabel(text) {
        ...
        // Hacemos el canvas un poco más ancho
        canvas.width = 256; canvas.height = 64;
        ctx.fillStyle = '#000'; ctx.fillRect(0,0,256,64);
        
        // Ajustamos el tamaño de la fuente según el largo de la palabra
        let fontSize = 40;
        if (text.length > 8) fontSize = 30;
        if (text.length > 12) fontSize = 20;
        ...
        sprite.scale.set(6, 1.5, 1); // Lo escalamos para que coincida con el nuevo ancho
        return sprite;
    }
    ...
    // --- 1. EL "FABRICANTE" DE SIGNIFICANTES ---
    // Función central que crea el significante (ahora recibe la palabra)
    const crearSignificante = (texto) => {
        sCount++;
        ...
        // Nace desde el Gran Otro
        mesh.position.set(
            granOtro.position.x + (Math.random() - 0.5) * 15,
            granOtro.position.y + (Math.random() - 0.5) * 15,
            granOtro.position.z + (Math.random() - 0.5) * 15
        );
        
        // Usamos la palabra pronunciada en lugar del S1, S2 genérico
        const label = createLabel(texto);
    ...
    // Mantenemos el click manual por si quieres agregar S1, S2, etc. sin hablar
    const manejarClick = () => {
        crearSignificante(`S${sCount + 1}`);
    };

    container.addEventListener('click', manejarClick);

    // --- SISTEMA DE RECONOCIMIENTO DE VOZ CON BOTÓN ---
    ...
    if (SpeechRecognition && btnMic) {
        ...
        // Efecto visual: cuando empieza a escuchar
        recognition.onstart = () => {
            btnMic.classList.add('grabando');
            console.log("Micrófono abierto. Te escucho...");
        };

        // Efecto visual: cuando se apaga
        recognition.onend = () => {
        ...
        recognition.onresult = (event) => {
            ...
            console.log("Se escuchó: ", transcript);
            
            const palabras = transcript.trim().split(/\s+/);
            
            // Disparamos cada palabra con 1 segundo de diferencia
            palabras.forEach((palabra, index) => {
                setTimeout(() => {
                    // Recordá que aquí llamamos a la función que renombramos arriba
                    crearSignificante(palabra);
                }, index * 1000); 
            });
        };
        ...
        // Disparador del micrófono al tocar el botón
        btnMic.addEventListener('click', (e) => {
            e.stopPropagation(); // Evita que se dispare un click en el fondo 3D
            recognition.start();
        });

    } else {
        console.warn("Tu navegador no soporta la Web Speech API.");
        if (btnMic) btnMic.style.display = 'none'; // Ocultamos el botón si no es compatible
    }
    ...
    function animate() {
        ...
        for (let i = 0; i < MAX_PARTICULAS; i++) {
            ...
            // Si la partícula NO está activa (antes de cualquier click/voz)
            if (!p || !p.active) {
                dummy.scale.set(0, 0, 0); // La hacemos invisible achicándola a cero
                dummy.updateMatrix();
